...
class Structure extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('parent_id, create_user_id, update_user_id, deleted, deleted_denied', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>100),
			array('create_date, update_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, parent_id, create_user_id, create_date, update_user_id, update_date, deleted, deleted_denied', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'parent' => array(self::BELONGS_TO,'Structure','parent_id'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'name' => 'Название',
			'parent_id' => 'Родетельская структурная единица',
